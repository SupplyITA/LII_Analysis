...
# ---------------------- il grader del prof manda un body json, mentre fastAPI si aspetta un url, quindi convertiamo ---------------
...
async def full_gs_eval(domain: str, db: Session = Depends(get_db)):
    ...
    for entry in gs_entries:
        try:
            ...
            parsed_data = await get_parsed_data(entry.url, html_text=html_salvato) # Questa modifica (ovvero l'aggiunta dell'html salvato) velocizza i controlli, fixa i problemi con l'F1 aggregato usando html pre scaricati
...
def get_db_stats(db: Session = Depends(get_db)):
    ...
    avg_eval_dict = {d: {"precision": 0.0, "recall": 0.0, "f1": 0.0} for d in domains}
    avg_eval_judge_dict = {d: 0.0 for d in domains}
